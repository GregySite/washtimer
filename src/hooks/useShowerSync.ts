...
export function useShowerSync(mode: 'parent' | 'child') {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`shower_${sessionCode}`)
      ...
      (payload) => {
        ...
        if (!data) return;
        // On met à jour l'état local avec ce qui vient de la base
  ...
  // Update sécurisé : accepte un code explicite pour éviter le bug du "2 clics"
  const updateSession = async (newData: any, codeOverride?: string) => {
    const code = codeOverride || sessionCode;
    if (!code) return;
    
    // Optimistic UI update (pour que l'écran change tout de suite)
    if (newData.status) setStatus(newData.status);

    await supabase.from('shower_sessions').update(newData).eq('session_code', code);
  };
