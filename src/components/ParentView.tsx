...
export default function ParentView() {
  ...
  const [localSteps] = useState(DEFAULT_STEPS);
  ...
  if (status === 'setup' || !sessionCode) {
    return (
      <div className="p-6 flex flex-col gap-6 items-center justify-center min-h-screen bg-slate-50">
        ...
        <Button size="lg" className="w-full h-20 text-2xl bg-blue-600" onClick={handleStart}>REJOINDRE ET LANCER</Button>
      </div>
    );
  }
    ...
    <div className="p-6 max-w-md mx-auto space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4">
        {status === 'running' ? (
          ...
          <Button className="h-20 bg-green-500 text-white" onClick={() => updateSession({ status: 'running' })}>
